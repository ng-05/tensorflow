...
fused_computation {
  param_0.2 = f32[8192,50304] parameter(0)
  bitcast = f32[4,2048,50304] bitcast(param_0.2)
  constant = f32[] constant(-inf)
  reduce = f32[8192] reduce(param_0.2, constant), dimensions={1}, to_apply=region
  ...
  ROOT subtract.1 = f32[4,2048,50304] subtract(subtract, broadcast.2)
}

ENTRY entry_computation {
  param_0.3 = f32[8192,50304] parameter(0)
  ROOT fusion = f32[4,2048,50304] fusion(param_0.3), kind=kCustom, calls=fused_computation, backend_config={"fusion_backend_config":{"kind":"__triton_softmax"}}
}
)"));
